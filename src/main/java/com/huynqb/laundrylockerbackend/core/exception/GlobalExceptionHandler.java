...
/**
 * Global exception handler for the application. Handles all exceptions and returns standardized
 * error responses.
 *
 * <p>Production-ready features: - Unique trace ID for each error - No stack trace exposure in
 * production - Detailed validation error handling - i18n support for error messages - Comprehensive
 * logging
 */
...
public class GlobalExceptionHandler {
  ...
  public ResponseEntity<ApiResponse<Void>> handleFirebaseAuth(
      FirebaseAuthException ex, HttpServletRequest request) {
    String traceId = generateTraceId();
  ...
  public ResponseEntity<ApiResponse<Void>> handleMethodArgumentNotValid(
    ...
    return ResponseEntity.badRequest()
        .body(
            buildErrorResponse(
                MessageConstants.E_COM002, messageService.get(MessageConstants.E_COM002), errors));
  }
  ...
  public ResponseEntity<ApiResponse<Void>> handleTypeMismatch(
    ...
    String traceId = generateTraceId();
    log.warn(
        "Type mismatch [traceId={}]: {} - expected {}",
        traceId,
        ex.getName(),
        ex.getRequiredType());
    ...
    List<Error> errors =
        List.of(
            Error.builder()
                ...
                .message(
                    String.format(
                        "Parameter '%s' should be of type %s", ex.getName(), expectedType))
  ...
  public ResponseEntity<ApiResponse<Void>> handleMethodNotSupported(
    ...
    String traceId = generateTraceId();
    log.warn(
        "Method not supported [traceId={}]: {} on {}",
        traceId,
        ex.getMethod(),
  ...
  public ResponseEntity<ApiResponse<Void>> handleHttpMessageNotReadable(
    ...
    return ResponseEntity.badRequest().body(buildErrorResponse("E_MALFORMED_REQUEST", message));
  }
  ...
  public ResponseEntity<ApiResponse<Void>> handleIllegalState(
    ...
    String message =
        isProductionProfile() ? "Operation not allowed in current state" : ex.getMessage();
    return ResponseEntity.status(HttpStatus.CONFLICT)
  ...
  /**
   * Handle all unhandled exceptions. In production, generic message is returned without exposing
   * internal details.
   */
