...
/**
 * Comprehensive sample data loader for dev and docker profiles.
 * Creates realistic data for all tables covering multiple scenarios.
 */
@Slf4j
@Configuration
...
public class SampleDataLoader {
    ...
    private List<User> createUsers(List<Role> roles) {
        ...
        // Admin
        users.add(createUser("admin@laundrylocker.com", "System", "Admin", "0900000001", pwd, Set.of(adminRole)));

        // Partners (3)
        users.add(createUser("partner.minh@gmail.com", "Minh", "Nguyễn Văn", "0900000002", pwd, Set.of(userRole, partnerRole)));
        users.add(createUser("partner.huong@gmail.com", "Hương", "Trần Thị", "0900000003", pwd, Set.of(userRole, partnerRole)));
        users.add(createUser("partner.nam@gmail.com", "Nam", "Lê Hoàng", "0900000004", pwd, Set.of(userRole, partnerRole)));

        // Customers (10)
        users.add(createUser("customer.huy@gmail.com", "Huy", "Lê Văn", "0901000001", pwd, Set.of(userRole)));
        users.add(createUser("customer.lan@gmail.com", "Lan", "Phạm Thị", "0901000002", pwd, Set.of(userRole)));
        users.add(createUser("customer.tuan@gmail.com", "Tuấn", "Võ Minh", "0901000003", pwd, Set.of(userRole)));
        users.add(createUser("customer.mai@gmail.com", "Mai", "Nguyễn Thị", "0901000004", pwd, Set.of(userRole)));
        users.add(createUser("customer.duc@gmail.com", "Đức", "Trần Văn", "0901000005", pwd, Set.of(userRole)));
        users.add(createUser("customer.linh@gmail.com", "Linh", "Hoàng Thị", "0901000006", pwd, Set.of(userRole)));
        users.add(createUser("customer.khoa@gmail.com", "Khoa", "Đặng Văn", "0901000007", pwd, Set.of(userRole)));
        users.add(createUser("customer.hang@gmail.com", "Hằng", "Bùi Thị", "0901000008", pwd, Set.of(userRole)));
        users.add(createUser("customer.long@gmail.com", "Long", "Phạm Hoàng", "0901000009", pwd, Set.of(userRole)));
        users.add(createUser("customer.thu@gmail.com", "Thu", "Lý Thị", "0901000010", pwd, Set.of(userRole)));

        // Staff-like user (for testing)
        users.add(createUser("staff.tung@gmail.com", "Tùng", "Nguyễn Văn", "0902000001", pwd, Set.of(userRole, staffRole)));

        return users;
    }

    private User createUser(String email, String firstName, String lastName, String phone, String pwd, Set<Role> roles) {
    ...
    private List<Store> createStores() {
        ...
        return List.of(
            ...
            storeRepository.save(Store.builder()
                ...
                .latitude(10.7731).longitude(106.7012)
                .status(StoreStatus.ACTIVE)
                .partner(p1).build()),
            storeRepository.save(Store.builder()
            ...
            storeRepository.save(Store.builder()
                ...
                .latitude(10.7756).longitude(106.6863)
                .status(StoreStatus.ACTIVE)
                .partner(p2).build()),
            storeRepository.save(Store.builder()
                ...
                .latitude(10.7780).longitude(106.6900)
                .status(StoreStatus.ACTIVE)
                .partner(p2).build())
        );
    }
    ...
    private List<Locker> createLockers() {
        ...
        lockers.add(createLockerWithBoxes("LOC-Q1-NH-01", "Tủ A - Nguyễn Huệ", s1, LockerStatus.ACTIVE,
            new BoxSize[]{BoxSize.SMALL, BoxSize.SMALL, BoxSize.MEDIUM, BoxSize.MEDIUM, BoxSize.MEDIUM, BoxSize.LARGE, BoxSize.LARGE, BoxSize.EXTRA_LARGE}));
        lockers.add(createLockerWithBoxes("LOC-Q1-NH-02", "Tủ B - Nguyễn Huệ", s1, LockerStatus.ACTIVE,
            new BoxSize[]{BoxSize.SMALL, BoxSize.MEDIUM, BoxSize.MEDIUM, BoxSize.LARGE, BoxSize.LARGE, BoxSize.EXTRA_LARGE}));

        // Store 2: 2 lockers
        Store s2 = allStores.get(1);
        lockers.add(createLockerWithBoxes("LOC-Q1-LL-01", "Tủ A - Lê Lợi", s2, LockerStatus.ACTIVE,
            new BoxSize[]{BoxSize.SMALL, BoxSize.MEDIUM, BoxSize.MEDIUM, BoxSize.LARGE, BoxSize.LARGE, BoxSize.EXTRA_LARGE}));
        lockers.add(createLockerWithBoxes("LOC-Q1-LL-02", "Tủ B - Lê Lợi", s2, LockerStatus.MAINTENANCE,
            new BoxSize[]{BoxSize.SMALL, BoxSize.MEDIUM, BoxSize.LARGE, BoxSize.EXTRA_LARGE}));

        // Store 4: 2 lockers
        Store s4 = allStores.get(3);
        lockers.add(createLockerWithBoxes("LOC-Q3-VVT-01", "Tủ A - Võ Văn Tần", s4, LockerStatus.ACTIVE,
            new BoxSize[]{BoxSize.SMALL, BoxSize.SMALL, BoxSize.MEDIUM, BoxSize.MEDIUM, BoxSize.LARGE, BoxSize.LARGE}));
        lockers.add(createLockerWithBoxes("LOC-Q3-VVT-02", "Tủ B - Võ Văn Tần", s4, LockerStatus.ACTIVE,
            new BoxSize[]{BoxSize.MEDIUM, BoxSize.MEDIUM, BoxSize.LARGE, BoxSize.LARGE, BoxSize.EXTRA_LARGE, BoxSize.EXTRA_LARGE}));

        // Store 5: 2 lockers
        Store s5 = allStores.get(4);
        lockers.add(createLockerWithBoxes("LOC-Q3-NDC-01", "Tủ A - Nguyễn Đình Chiểu", s5, LockerStatus.ACTIVE,
            new BoxSize[]{BoxSize.SMALL, BoxSize.MEDIUM, BoxSize.MEDIUM, BoxSize.LARGE, BoxSize.EXTRA_LARGE}));
        lockers.add(createLockerWithBoxes("LOC-Q3-NDC-02", "Tủ B - Nguyễn Đình Chiểu", s5, LockerStatus.DISCONNECTED,
            new BoxSize[]{BoxSize.MEDIUM, BoxSize.LARGE, BoxSize.LARGE, BoxSize.EXTRA_LARGE}));

        return lockers;
    }

    private Locker createLockerWithBoxes(String code, String name, Store store, LockerStatus status, BoxSize[] sizes) {
        Locker locker = lockerRepository.save(Locker.builder()
            .code(code).name(name).store(store).status(status).build());

        for (int i = 0; i < sizes.length; i++) {
            boxRepository.save(Box.builder()
                .locker(locker).boxNumber(i + 1).size(sizes[i])
                .status(BoxStatus.AVAILABLE).isActive(true).build());
        }
        return locker;
    }
    ...
    private List<LaundryService> createServices() {
        ...
        // Storage services
        services.add(saveService("Gửi hàng thường", "Gửi đồ vào tủ với phí cơ bản", 5000, null, "lần", ServiceType.STANDARD_DROPOFF, false, false, 0, mainStore));
        services.add(saveService("Gửi qua đêm", "Phí phụ thu khi gửi qua đêm", 5000, null, "đêm", ServiceType.OVERNIGHT, true, false, 0, mainStore));
        services.add(saveService("Gửi nhanh 2h", "Lấy đồ trong vòng 2 tiếng", 10000, null, "lần", ServiceType.EXPRESS_2H, false, false, 2, mainStore));

        // Laundry services
        services.add(saveService("Giặt sấy thường", "Giặt và sấy khô quần áo thông thường", 12000, 15000, "kg", ServiceType.LAUNDRY, false, false, 24, mainStore));
        services.add(saveService("Giặt hấp cao cấp", "Giặt hấp cho đồ cao cấp, vest, áo dài", 25000, 35000, "món", ServiceType.LAUNDRY, false, false, 48, mainStore));
        services.add(saveService("Giặt đồ lớn", "Chăn, mền, rèm cửa", 30000, 50000, "món", ServiceType.LAUNDRY, false, false, 48, mainStore));
        services.add(saveService("Giặt giày/túi", "Giặt giày, túi xách", 40000, 80000, "đôi/cái", ServiceType.LAUNDRY, false, false, 72, mainStore));
        services.add(saveService("Là ủi", "Chỉ là ủi, không giặt", 8000, 12000, "món", ServiceType.LAUNDRY, false, false, 4, mainStore));

        // Monthly packages
        services.add(saveService("Gói tháng Sinh viên", "Giặt không giới hạn cho sinh viên", 50000, null, "tháng", ServiceType.MONTHLY_STUDENT, false, true, 0, mainStore));
        services.add(saveService("Gói tháng Shipper", "Giặt không giới hạn cho shipper", 70000, null, "tháng", ServiceType.MONTHLY_SHIPPER, false, true, 0, mainStore));

        return services;
    }

    private LaundryService saveService(String name, String desc, int price, Integer maxPrice, String unit,
                                       ServiceType type, boolean isAddon, boolean isMonthly, int hours, Store store) {
        return serviceRepository.save(LaundryService.builder()
            .name(name).description(desc).price(new BigDecimal(price))
            .maxPrice(maxPrice != null ? new BigDecimal(maxPrice) : null)
            .unit(unit).serviceType(type).status(ServiceStatus.ACTIVE)
            .isAddon(isAddon).isMonthlyPackage(isMonthly).estimatedHours(hours)
            .store(store).build());
    }
    ...
    private List<Order> createOrders() {
        ...
        // ===== COMPLETED Orders (6) =====
        orders.add(createFullOrder(huy, locker1, null, null, OrderStatus.COMPLETED, null, 2.5, 42500, "Giặt quần áo hàng ngày", -7, -5, storage, laundry));
        orders.add(createFullOrder(lan, locker1, null, null, OrderStatus.COMPLETED, null, 1.8, 32000, "Giặt đồ công sở", -14, -12, storage, laundry));
        orders.add(createFullOrder(tuan, locker2, null, null, OrderStatus.COMPLETED, null, 4.0, 75000, "Giặt đồ gia đình", -21, -19, storage, laundry));
        orders.add(createFullOrder(linh, locker1, null, null, OrderStatus.COMPLETED, null, 1.0, 30000, "Giặt áo vest", -10, -8, storage, premium));
        orders.add(createFullOrder(khoa, locker3, null, null, OrderStatus.COMPLETED, null, 3.2, 58000, "Giặt chăn mền", -5, -3, storage, large));
        orders.add(createFullOrder(hang, locker2, null, null, OrderStatus.COMPLETED, null, 2.0, 39000, "Giặt thường", -3, -1, storage, laundry));

        // ===== CANCELED Orders (2) =====
        orders.add(createFullOrder(thu, locker1, null, null, OrderStatus.CANCELED, null, null, 0, "Đơn hủy do khách không đến", -2, null, storage, laundry));
        orders.add(createFullOrder(longUser, locker2, null, null, OrderStatus.CANCELED, null, null, 0, "Khách yêu cầu hủy", -4, null, storage, premium));

        // ===== WAITING Orders (3) =====
        Box box1 = boxes.get(0); box1.setStatus(BoxStatus.OCCUPIED); boxRepository.save(box1);
        orders.add(createFullOrder(lan, locker1, box1, null, OrderStatus.WAITING, "123456", null, 65000, "Giặt áo vest và quần tây", 0, null, storage, premium));

        Box box2 = boxes.get(1); box2.setStatus(BoxStatus.OCCUPIED); boxRepository.save(box2);
        orders.add(createFullOrder(duc, locker1, box2, null, OrderStatus.WAITING, "234567", null, 45000, "Giặt đồ thể thao", 0, null, storage, laundry));

        List<Box> boxes3 = boxRepository.findByLockerId(allLockers.get(4).getId());
        Box box3 = boxes3.get(0); box3.setStatus(BoxStatus.OCCUPIED); boxRepository.save(box3);
        orders.add(createFullOrder(linh, locker3, box3, null, OrderStatus.WAITING, "345678", null, 80000, "Giặt chăn", 0, null, storage, large));

        // ===== COLLECTED Orders (2) =====
        orders.add(createFullOrder(khoa, locker1, null, null, OrderStatus.COLLECTED, null, 2.5, 47000, "Đã lấy, đang cân", 0, null, storage, laundry));
        orders.add(createFullOrder(hang, locker2, null, null, OrderStatus.COLLECTED, null, 1.5, 35000, "Đang xử lý tại xưởng", 0, null, storage, laundry));

        // ===== PROCESSING Orders (2) =====
        orders.add(createFullOrder(tuan, locker1, null, null, OrderStatus.PROCESSING, null, 3.0, 55000, "Đang giặt chăn mền", -1, null, storage, large));
        orders.add(createFullOrder(mai, locker3, null, null, OrderStatus.PROCESSING, null, 2.2, 41000, "Đang giặt", -1, null, storage, laundry));

        // ===== READY Orders (2) =====
        orders.add(createFullOrder(huy, locker1, null, null, OrderStatus.READY, null, 1.8, 33000, "Đã giặt xong, chờ trả", -1, null, storage, laundry));
        orders.add(createFullOrder(longUser, locker2, null, null, OrderStatus.READY, null, 2.5, 48000, "Sẵn sàng trả về tủ", -1, null, storage, laundry));

        // ===== RETURNED Orders (2) =====
        Box box4 = boxes.get(3); box4.setStatus(BoxStatus.OCCUPIED); boxRepository.save(box4);
        orders.add(createFullOrder(mai, locker1, null, box4, OrderStatus.RETURNED, "654321", 1.5, 32500, "Đồ đã về tủ, chờ lấy", -1, null, storage, laundry));

        Box box5 = boxes.get(4); box5.setStatus(BoxStatus.OCCUPIED); boxRepository.save(box5);
        orders.add(createFullOrder(thu, locker1, null, box5, OrderStatus.RETURNED, "765432", 2.0, 39000, "Chờ khách thanh toán", -1, null, storage, laundry));

        // ===== INITIALIZED Order (1) =====
        List<Box> boxes2 = boxRepository.findByLockerId(allLockers.get(1).getId());
        Box box6 = boxes2.get(0); box6.setStatus(BoxStatus.RESERVED); boxRepository.save(box6);
        orders.add(createFullOrder(duc, allLockers.get(1), box6, null, OrderStatus.INITIALIZED, "111222", null, 17000, "Mới tạo, chưa bỏ đồ", 0, null, storage, laundry));

        return orders;
    }

    private Order createFullOrder(User sender, Locker locker, Box sendBox, Box receiveBox, OrderStatus status,
                                  String pin, Double weight, int totalPrice, String desc,
                                  int createdDaysAgo, Integer completedDaysAgo,
                                  LaundryService... services) {
        Order order = orderRepository.save(Order.builder()
            .type(OrderType.LAUNDRY).sender(sender).locker(locker)
            .sendBox(sendBox).receiveBox(receiveBox).status(status)
            .pinCode(pin).pinCodeIssuedAt(pin != null ? LocalDateTime.now() : null)
            .actualWeight(weight != null ? new BigDecimal(weight) : null)
            .weightUnit(weight != null ? "kg" : null)
            .reservationFee(new BigDecimal("5000")).storagePrice(new BigDecimal("5000"))
            .totalPrice(new BigDecimal(totalPrice)).description(desc)
            .completedAt(completedDaysAgo != null ? LocalDateTime.now().minusDays(-completedDaysAgo) : null)
            .build());
        ...
        for (int i = 0; i < services.length; i++) {
            orderDetailRepository.save(OrderDetail.builder()
                .order(order).service(services[i])
                .quantity(i == 0 ? 1.0 : (weight != null ? weight : 1.0))
                .price(services[i].getPrice()).build());
        }
    ...
    private int createPayments() {
        ...
        for (Order order : completedOrders) {
            paymentRepository.save(Payment.builder()
                .order(order).customer(order.getSender())
        ...
        Order returnedOrder = allOrders.stream()
            .filter(o -> o.getStatus() == OrderStatus.RETURNED)
            .findFirst().orElse(null);
        if (returnedOrder != null) {
            paymentRepository.save(Payment.builder()
                .order(returnedOrder).customer(returnedOrder.getSender())
        ...
        // Failed payment
        paymentRepository.save(Payment.builder()
            .order(allOrders.get(0)).customer(allOrders.get(0).getSender())
    ...
    private int createAccessCodes() {
        ...
        List<Order> waitingOrders = allOrders.stream()
        ...
        for (int i = 0; i < Math.min(waitingOrders.size(), codes.length); i++) {
            accessCodeRepository.save(StaffAccessCode.builder()
                .code(codes[i]).order(waitingOrders.get(i))
                .partner(i % 2 == 0 ? partner1 : partner2)
                .action(AccessCodeAction.COLLECT)
                .status(AccessCodeStatus.ACTIVE)
                .expiresAt(LocalDateTime.now().plusHours(24))
                .notes("Mã cho nhân viên lấy đồ").build());
            count++;
        }

        // Used code
        Order collectedOrder = allOrders.stream()
            .filter(o -> o.getStatus() == OrderStatus.COLLECTED)
            .findFirst().orElse(null);
        if (collectedOrder != null) {
            accessCodeRepository.save(StaffAccessCode.builder()
                .code("USED1234").order(collectedOrder).partner(partner1)
                .action(AccessCodeAction.COLLECT)
                .status(AccessCodeStatus.USED)
                .usedAt(LocalDateTime.now().minusHours(2))
                .staffName("Nguyễn Văn Tùng").build());
            count++;
        }

        // Expired code
        accessCodeRepository.save(StaffAccessCode.builder()
            .code("EXPIRED1").order(allOrders.get(0)).partner(partner1)
            .action(AccessCodeAction.COLLECT)
        ...
        Order readyOrder = allOrders.stream()
            .filter(o -> o.getStatus() == OrderStatus.READY)
            .findFirst().orElse(null);
        if (readyOrder != null) {
            accessCodeRepository.save(StaffAccessCode.builder()
                .code("RET12345").order(readyOrder).partner(partner1)
                .action(AccessCodeAction.RETURN)
                .status(AccessCodeStatus.ACTIVE)
                .expiresAt(LocalDateTime.now().plusHours(12))
                .notes("Mã trả đồ về tủ").build());
            count++;
        }
    ...
    private int createLoyaltyAccounts() {
        ...
        BigDecimal[] spent = {
            new BigDecimal("500000"), new BigDecimal("1200000"), new BigDecimal("200000"),
            new BigDecimal("3000000"), BigDecimal.ZERO, new BigDecimal("850000"),
            new BigDecimal("1500000"), new BigDecimal("100000"), new BigDecimal("2200000"),
    ...
    private int createStampCards() {
        ...
        for (int i = 0; i < Math.min(5, accounts.size()); i++) {
            ...
            // Box stamp card
            StampCard boxCard = stampCardRepository.save(StampCard.builder()
                .user(user).stampType(StampType.BOX)
                .boxSize("MEDIUM").stampsRequired(6)
                .currentStamps(i + 1)
            ...
            // Transaction
            stampTransactionRepository.save(StampTransaction.builder()
                .user(user).stampCard(boxCard)
        ...
        if (accounts.size() > 3) {
            ...
            StampCard svcCard = stampCardRepository.save(StampCard.builder()
                .user(user).stampType(StampType.SERVICE)
                .service(allServices.get(3))
                .stampsRequired(6).currentStamps(0)
            ...
            stampTransactionRepository.save(StampTransaction.builder()
                .user(user).stampCard(svcCard)
                .type(StampTransactionType.REDEEM)
                .stamps(-6).stampsAfter(0).rewardsAfter(0)
    ...
    private int createNotifications() {
        ...
        for (Payment payment : payments.subList(0, Math.min(3, payments.size()))) {
            notificationRepository.save(Notification.builder()
                ...
                .title(payment.getStatus() == PaymentStatus.COMPLETED ? "Thanh toán thành công" : "Thanh toán thất bại")
                .message("Đơn hàng #" + payment.getOrder().getId() + " - " + payment.getAmount() + " VND")
    ...
    private void printSummary() {
        ...
        log.info("│ Point Transactions  │ {}                     │", pointTransactionRepository.count());
